# YouTube 화면 프레임 분석 요약 Skill

YouTube URL 또는 로컬 영상 파일 하나를 입력하면 **자막만 읽는 것이 아니라 화면 프레임까지 함께 분석**해 한국어 마크다운 요약을 작성하는 Skill입니다. 영상 속 슬라이드, 제품 UI, 코드 화면, 다이어그램처럼 자막에 직접 나오지 않는 시각 정보도 요약 근거로 사용합니다.

핵심 흐름은 단순합니다. 스크립트가 자막/STT, 메타데이터, 댓글, 대표 프레임을 디스크에 준비하고, Codex는 `manifest.json`을 먼저 읽은 뒤 필요한 자막 구간과 화면 프레임만 열어 최종 요약을 작성합니다.
